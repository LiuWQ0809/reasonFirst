...
本地 MCP 在用户登录后由 `com.reasonfirst.v4-mcp` 自动启动，地址为 `http://127.0.0.1:8765/mcp`。Codex、ChatGPT Desktop 插件和网页控制 relay 共用这个 MCP 进程及 `~/.local/share/reasonfirst/codex-web-bridge/state.json`。
...
安装程序会通过 Codex CLI 安装并启用个人插件。安装后重启 ChatGPT Desktop/Codex，以重新读取插件与 MCP 配置。
...
若以后开通开发者模式，可设置 `RF_TUNNEL_ID` 与 `CONTROL_PLANE_API_KEY` 后重跑 `configure_v4.sh`，通过 Secure MCP Tunnel 将同一 MCP URL 接入 Web。Tunnel 客户端从 OpenAI 官方发布页下载安装并校验 SHA256；密钥保存在 macOS Keychain，不写入包内。此路线目前不是必需项。
